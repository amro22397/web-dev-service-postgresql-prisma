...
const ThemeChanger = ({ className }: { className?: string }) => {
  ...
  if (resolvedTheme === 'dark') {
    return <FiSun className={`fi-sun ${className ? className : "sun-moon-classname-arabic"}`} size={25}
    onClick={() => setTheme('light')} />
  }
  
  if (resolvedTheme === 'light') {
    return <FiMoon className={`fi-moon ${className ? className : "sun-moon-classname-arabic"}`} size={25}
